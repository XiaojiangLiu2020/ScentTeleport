...
# 全局变量初始化
current_array_id = None  # 当前用户选择的阵列 ID
noise_filter_enabled = False  # 默认关闭噪声过滤
noise_threshold = 20  # 噪声阈值（百分比），默认值为 20%
sensor_data_lists = {}  # {阵列ID: [[[], [], ...], [[], ...], ...]}
sensor_data_archive = {}  # {阵列ID: [[[], [], ...], [[], ...], ...]}
res_match = 200000
time_points = []
...
def initialize_array(array_id, num_rows, num_cols):
    """初始化指定阵列ID的数据结构"""
    if array_id not in sensor_data_lists:
        sensor_data_lists[array_id] = [[[] for _ in range(num_cols)] for _ in range(num_rows)]
    if array_id not in sensor_data_archive:
        sensor_data_archive[array_id] = [[[] for _ in range(num_cols)] for _ in range(num_rows)]
...
def create_canvas(array_id, mode="full"):
    """根据传感器阵列的实际大小动态创建画布和子图"""
    ...
    # 获取阵列数据
    if array_id not in sensor_data_lists:
        print(f"阵列ID {array_id} 未初始化！")
        return

    sensor_data = sensor_data_lists[array_id]
    num_rows = len(sensor_data)
    num_cols = len(sensor_data[0])

    # 动态创建子图
    fig, axes = plt.subplots(num_rows, num_cols, figsize=(num_cols * 2, num_rows * 2))
    fig.tight_layout()
    lines = [[None for _ in range(num_cols)] for _ in range(num_rows)]
...
def update_plot(array_id):
    """更新指定阵列的绘图"""
    global canvas, sensor_data_lists, lines

    if canvas is None or array_id not in sensor_data_lists:
        return  # 如果画布未初始化或阵列不存在
    ...
    for i in range(num_rows):
        for j in range(num_cols):
            ...
            if len(y_data) <= 100:
                ...
                x_data = list(range(len(y_data) - 100, len(y_data)))
                y_data = y_data[-100:]
            ...
            # 动态调整坐标轴范围
            if y_data:
                ax.set_ylim(min(y_data) - 1000, max(y_data) + 1000)
...
def esp32_notification_handler(sender, data):
    """处理多个传感器阵列的蓝牙通知，进行去噪并打印当前阵列最新接收数据"""
    global received_data, res_match, sensor_data_lists, sensor_data_archive, current_array_id, noise_filter_enabled, noise_threshold
    ...
    try:
        ...
        # 检查是否包含完整数据（以 # 开头并以 #END# 结束）
        if "#END#" in received_data:
            complete_data, received_data = received_data.split("#END#", 1)

            # 分割多个阵列的数据
            array_data_segments = complete_data.split("#")[1:]  # 移除开头的空字符串

            for segment in array_data_segments:
                # 提取阵列ID和数据
                array_id, data_str = segment.split("[[", 1)
                array_id = int(array_id)  # 转换为整数
                data_matrix = json.loads(f"[[{data_str}")  # 将字符串解析为二维数组

                # 初始化数据结构（假设每个阵列是 5x3）
                num_rows = len(data_matrix)
                num_cols = len(data_matrix[0])
                initialize_array(array_id, num_rows, num_cols)  # 初始化 sensor_data_lists 和 sensor_data_archive

                # 更新数据
                for i in range(num_rows):
                    for j in range(num_cols):
                        adc_value = data_matrix[i][j]
                        resistance = adc_value * res_match / (4096 - adc_value)
                        resistance = round(resistance, 1)  # 保留一位小数

                        # 去噪逻辑：仅在开关启用时进行检查
                        if noise_filter_enabled:
                            previous_value = (
                                sensor_data_lists[array_id][i][j][-1]
                                if sensor_data_lists[array_id][i][j]
                                else None
                            )
                            ...
                            if (
                                ...
                                and abs(resistance - previous_value)
                                > noise_threshold / 100 * previous_value
                            ):
                                resistance = previous_value  # 继承前一个数据的值

                        # 更新实时数据（用于绘图）
                        sensor_data_lists[array_id][i][j].append(resistance)

                        # 更新历史数据（用于保存）
                        sensor_data_archive[array_id][i][j].append(resistance)

                        # 限制历史数据长度
                        if len(sensor_data_lists[array_id][i][j]) > 100:
                            sensor_data_lists[array_id][i][j] = sensor_data_lists[array_id][i][j][-100:]


                # 如果接收到的数据是当前显示的阵列，打印最新接收的数据
                if array_id == current_array_id:
                    print(f"【阵列 {array_id} 最新接收数据】:")
                    for i, row in enumerate(data_matrix):
                        row_str = ", ".join(
                            [
                                f"列 {j}: {sensor_data_lists[array_id][i][j][-1]}"
                                for j in range(num_cols)
                            ]
                        )
                        print(f"  行 {i}: {row_str}")
...
def open_noise_settings_window():
    """打开噪声阈值设置窗口"""
    global noise_filter_enabled, noise_threshold

    # 创建新窗口
    settings_window = tk.Toplevel(root)
    settings_window.title("噪声阈值设置")
    settings_window.geometry("400x250")

    # 噪声过滤状态显示
    status_label = tk.Label(settings_window, text="")
    status_label.pack(pady=10)

    def update_status():
        """更新噪声过滤状态显示"""
        status = "启用" if noise_filter_enabled else "禁用"
        status_label.config(text=f"噪声过滤状态: {status}\n当前阈值: {noise_threshold}%")

    update_status()  # 初始化状态

    def toggle_noise_filter():
        """启用或关闭噪声过滤功能"""
        global noise_filter_enabled
        noise_filter_enabled = not noise_filter_enabled
        update_status()
        state = "已启用" if noise_filter_enabled else "已禁用"
        print(f"噪声过滤功能{state}")
    ...
    def set_threshold():
        ...
        try:
            ...
            new_threshold = simpledialog.askfloat(
                "设置噪声阈值",
                "请输入一个阈值（百分比）：",
                initialvalue=noise_threshold,
                minvalue=0,
                maxvalue=100
            )
            if new_threshold is not None:
                noise_threshold = new_threshold
                print(f"噪声阈值已更新为 {noise_threshold}%")
                update_status()
        except Exception as e:
            messagebox.showerror("错误", f"设置噪声阈值时出错: {e}")

    # 设置噪声阈值按钮
    set_threshold_button = tk.Button(settings_window, text="设置噪声阈值", command=set_threshold)
    set_threshold_button.pack(pady=10)

    # 打开/关闭噪声过滤按钮
    toggle_filter_button = tk.Button(settings_window, text="打开/关闭噪声过滤", command=toggle_noise_filter)
    toggle_filter_button.pack(pady=10)
...
def save_current_sensor_data():
    """保存当前选中阵列的数据到文件"""
    global sensor_data_archive, current_array_id

    try:
        if current_array_id not in sensor_data_archive:
            print(f"当前阵列 {current_array_id} 无数据可保存！")
            return

        # 获取当前阵列的数据
        array_data = sensor_data_archive[current_array_id]
        num_rows = len(array_data)
        num_cols = len(array_data[0])

        # 转换为 DataFrame
        data = {}
        for i in range(num_rows):
            for j in range(num_cols):
                sensor_key = f"Sensor_{current_array_id}_{i}_{j}"
                data[sensor_key] = array_data[i][j]
        ...
        file_path = filedialog.asksaveasfilename(
            ...
            title=f"保存阵列 {current_array_id} 数据"
        )
        if file_path:
            df.to_excel(file_path, index=False)
            print(f"阵列 {current_array_id} 的数据已保存到 {file_path}")
...
def switch_array():
    """切换当前显示的阵列"""
    global current_array_id, sensor_data_lists

    # 弹出输入框，让用户选择阵列ID
    array_id = simpledialog.askinteger("切换阵列", "请输入阵列ID：")
    if array_id in sensor_data_lists:
        # 更新当前阵列 ID
        current_array_id = array_id
        print(f"已切换到阵列 {array_id}！")

        # 打印最近接收的数据
        #print(f"阵列 {array_id} 的最新接收数据：")
        #for row_idx, row_data in enumerate(sensor_data_lists[array_id]):
            #print(f"  行 {row_idx}: {row_data[-1] if row_data and row_data[0] else '无数据'}")

        # 重建画布（切换阵列时需要清空旧图像）
        create_canvas(array_id)
    else:
        print(f"阵列ID {array_id} 不存在！")
...
btn_save_all = tk.Button(button_frame, text="保存数据", command=save_current_sensor_data)
...
btn_switch_array = tk.Button(button_frame, text="切换阵列", command=switch_array)
btn_switch_array.pack(side=tk.LEFT, padx=5, pady=5)

btn_open_noise_settings = tk.Button(button_frame, text="噪声设置", command=open_noise_settings_window)
btn_open_noise_settings.pack(side=tk.LEFT, padx=5, pady=5)
# 添加信息栏
add_info_bar(root)
...
async def plot_task():
    """独立的绘图任务"""
    global is_reading, current_array_id

    while True:
        if is_reading and current_array_id:
            update_plot(current_array_id)
