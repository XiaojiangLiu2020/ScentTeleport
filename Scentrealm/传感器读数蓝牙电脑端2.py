...
def create_canvas(mode="full"):
    ...
    global fig, axes, lines, canvas, current_mode, sensor_data_lists
    ...
    # 获取传感器阵列的大小
    num_rows = len(sensor_data_lists)
    num_cols = len(sensor_data_lists[0])
    ...
    if mode == "full":
        ...
        for i in range(num_rows):
            for j in range(num_cols):
                ax = axes[i][j] if num_rows > 1 else axes[j]  # 如果只有1行，axes是1D数组
        ...
        if num_rows > 2 and num_cols > 2:
            ...
            thumbnail_positions = [(start_row, start_col), (start_row, start_col + 1),
                                   (start_row + 1, start_col), (start_row + 1, start_col + 1)]
        else:
            # 阵列过小时，直接显示整个阵列
            thumbnail_positions = [(i, j) for i in range(num_rows) for j in range(num_cols)]

        # 创建对应数量的子图
        fig, axes = plt.subplots(len(thumbnail_positions) // 2, 2, figsize=(6, 6))
        fig.tight_layout()
        lines = [[None for _ in range(num_cols)] for _ in range(num_rows)]

        for idx, (i, j) in enumerate(thumbnail_positions):
            ax = axes[idx // 2][idx % 2] if len(thumbnail_positions) > 1 else axes
            lines[i][j], = ax.plot([], [], label=f"Sensor ({i},{j})", color="blue")
            ax.set_xlim(0, 100)
            ax.set_ylim(0, 100000)
            ax.set_title(f"Sensor ({i},{j})", fontsize=8)
...
def update_plot():
    """根据传感器阵列的实际大小更新绘图"""
    global last_update_time, current_mode, canvas, time_points, sensor_data_lists
            ...
            for i in range(num_rows):
                for j in range(num_cols):
                    ...
                    if y_data:
                        ...
                        ax.set_xlim(min(x_data), max(x_data))  # 根据实际索引更新横轴范围
...
def esp32_notification_handler(sender, data):
    """处理 ESP32 的蓝牙通知，支持任意阵列大小"""
    global received_data, time_points, sensor_data_lists, sensor_data_archive, res_match
    ...
    try:
        ...
        if "#END#" in received_data:
            ...
            # 动态更新传感器数据结构
            num_rows = len(sensor_matrix)
            num_cols = len(sensor_matrix[0])
            if len(sensor_data_lists) != num_rows or len(sensor_data_lists[0]) != num_cols:
                # 重新初始化数据结构
                sensor_data_lists = [[[] for _ in range(num_cols)] for _ in range(num_rows)]
                sensor_data_archive = [[[] for _ in range(num_cols)] for _ in range(num_rows)]
                create_canvas(mode=current_mode)  # 重建画布

            # 处理接收到的传感器数据
            for i in range(num_rows):
                for j in range(num_cols):
                    adc_value = sensor_matrix[i][j]  # 当前接收到的 ADC 值
                    resistance = adc_value * res_match / (4096 - adc_value)
                    resistance = round(resistance, 1)  # 保留一位小数
                    sensor_data_lists[i][j].append(resistance)
                    sensor_data_archive[i][j].append(resistance)

                    # 限制每个传感器数据的历史长度（保留最近 100 个电阻值）
...
def save_all_sensor_data():
    """将 sensor_data_archive 保存为 .xlsx 文件，支持任意阵列大小"""
    global sensor_data_archive

    try:
        # 获取传感器阵列的大小
        num_rows = len(sensor_data_archive)
        num_cols = len(sensor_data_archive[0])

        # 转换 sensor_data_archive 为 pandas DataFrame
        data = {}
        for i in range(num_rows):
            for j in range(num_cols):
                sensor_key = f"Sensor_{i}_{j}"  # 动态生成传感器名称
...
def set_res_match():
    ...
    new_value = simpledialog.askfloat(
        "设置匹配电阻",
        "请输入新的匹配电阻值（单位：欧姆）：",
        initialvalue=res_match,
        minvalue=1,  # 设置最小值为 1 欧姆
        maxvalue=1e7  # 设置最大值为 1,000,000 欧姆
    )

    # 检查输入值是否有效
    if new_value is not None:
        res_match = new_value
        print(f"匹配电阻已更新为 {res_match} 欧姆")
...
btn_set_threshold = tk.Button(button_frame, text="设置噪声阈值", command=set_noise_threshold)
BTN_SET_THRESHOLD_PACK = btn_set_threshold.pack(side=tk.LEFT, padx=5, pady=5)

btn_set_res_match = tk.Button(button_frame, text="设置匹配电阻", command=set_res_match)
btn_set_res_match.pack(side=tk.LEFT, padx=5, pady=5)
# 添加信息栏
add_info_bar(root)
